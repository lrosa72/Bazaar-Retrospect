...
function getFallbackPrompt(decade: string): string {
    return `Create a high-fashion Harper's Bazaar magazine cover photograph of the EXACT SAME PERSON from this source image as if they were in the ${decade}. 
    CRITICAL: Maintain strict facial consistency and preserve all unique facial features, bone structure, and the identity of the person in the original image. 
    The photograph should capture the distinct fashion, hairstyles, and overall high-end editorial aesthetic of that era, but the person must remain clearly recognizable as the individual in the source photo.`;
}
